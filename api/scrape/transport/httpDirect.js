// Level 1: TLS-fingerprinted HTTP via impit
// Impersonates Chrome 120 at the JA3/JA4 level
...
async function scrape(url) {
  ...
  if (!client) {
    return { ok: false, error: 'impit not available', level: 1 };
  }
  ...
  for (const buildUrl of PAGES_TO_CHECK) {
    ...
    try {
      const resp = await client.fetch(pageUrl, {
  ...
  if (result.pages_scanned.length === 0) {
    return { ok: false, error: 'all pages failed', level: 1 };
  }

  return { ok: true, data: result, level: 1 };
}
